...
```
Error: locator.click: Error: strict mode violation: getByText('File Download') resolved to 2 elements:
    1) <a href="/download">File Download</a> aka getByRole('link', { name: 'File Download', exact: true })
    2) <a href="/download_secure">Secure File Download</a> aka getByRole('link', { name: 'Secure File Download' })

Call log:
  - waiting for getByText('File Download')
...
```ts
  ...
  5  | 
> 6  |     await page.getByText('File Download').click();
     |                                           ^ Error: locator.click: Error: strict mode violation: getByText('File Download') resolved to 2 elements:
  7  |    const downPromise=page.waitForEvent('download')
  8  | 
  9  |    await page.getByText('myfile.txt').click()
  10 |     
  11 | const download=await downPromise;
  12 | 
  13 | console.log(await download.suggestedFilename())
  14 | 
  15 | 
  16 | 
  17 | 
